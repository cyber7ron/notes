...
**Tools used to enumeration**

1. [link to Powerview.ps1](https://github.com/PowerShellMafia/PowerSploit/blob/master/Recon/PowerView.ps1)
...
trust is relationship between two domain or forest which allows users of one doman or forest to access resources in the other domain or forest.

trust can be automatic or established.

Trusted Domain Objets(TDO) represent the trust relationships in a domain.

* Trust Direction :
	* One-way-trust : Unidrectional. users in trusted domain can access resources in the trusting domain but the reverse is not true.
...
if `domain` A is in birectional trust with `domain B` ,and `domain B` is in bidirectional trust with `domain c` , then `domain c` and `domain a` is in bidirectional trust.

2. Non-transitivity :

cannot be extended to other domain in the forest,Can be two-way or one-way. 

* Type of Trusts:
	* Default/Automatic Trusts
	microsoft consider forest for security boundry, that means all domain in the forest have automatically bidirectional trust.there is no need to create trust manually.
		* Parent-child trust :  
		its create automatically between new domain and the domain that precedes it in hierarchy, whenever new domain added in tree. this trust is always two way transitive.

		* Tree-root trust : its create automatically between whenever new domain tree is added to a forest root. this is always two-way transitive.
		

	* Shortcut Trust: used to reduce access times in complex trust scenarios, can be one-way or two-way transitive.
	

	* External Trust : between two domain in different forests when forests do not have a trust relationship. it canb e one-way or two-way and is nontransitive.
	

	* Forest trust : between forest root domains. cannot be extended to a third forest. means if `forest 1` and `forest 2` have a trust and `forest 2` and `forest 3` have a trust then `forest1` and `forest 3` does not have implicit trust.
