...
you get elevated shell on remote system if admin creds are used to authenticate.(which is the default settings.)

* There is two type of powershell remoting
...
2. One to Many
	...
	* Non-interactive
	* Executes cmds parallely.
	* Usefull cmdlets
		`Invoke-Command`

in One to Many type we can : 

* Run cmds and scripts on
	- multiple remote computers
	- in disconnected sessions
	- as background job and more
* pass the hash, using creds and execute cmd on multiple remote computer.
* Use `-Credential` parameter to pass username/password.


#### Invoke-Mimikatz

the script could be used to dump credentials, tickets and more. We can use mimikatz with  powershell wothout dropping the mimikatz exe to disk.

its usefull for passing the hash and replaying hashes, tickets and for many exciting active directory attacks.

Using the code from ReflectivePEInjectoins, mimikatz is loaded reflectively into the memory. All the function of mimikatz could be used from this script.

the script needs administrative privileges for dumping creds from local machine, many attacks need specific privs which are covered while discussing that attacks.
